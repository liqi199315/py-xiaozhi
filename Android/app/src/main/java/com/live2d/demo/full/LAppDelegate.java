...
public class LAppDelegate {
    ...
    public void release() {
        onStop();
        // CubismFramework.dispose() is called in onStop()
    }

    public void onDestroy() {
        // No-op, cleanup is handled in release()
    }

    public void onSurfaceCreated() {
        // Initialize Cubism SDK framework FIRST
        ...
        // 强制重新初始化管理器和模型，确保 GL 资源在当前 context 下重新创建
        LAppLive2DManager.releaseInstance();
    ...
    private LAppDelegate() {
        ...
        // Only start up if not already initialized to avoid race conditions
        try {
            CubismFramework.startUp(cubismOption);
        } catch (Exception e) {
            LAppPal.printLog("CubismFramework.startUp failed or already started: " + e.getMessage());
        }
    }
