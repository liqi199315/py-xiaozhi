...
public class LAppLive2DManager {
    ...
    public void setUpModel() {
        ...
        if (LAppDelegate.getInstance().getActivity() == null) {
            LAppPal.printLog("LAppDelegate activity is null, skipping setUpModel");
            return;
        }
    ...
    public void changeScene(int index) {
        if (modelDir.size() <= index) {
            LAppPal.printLog("modelDir index out of bounds: " + index);
            return;
        }
        currentModel = index;
        if (DEBUG_LOG_ENABLE) {
